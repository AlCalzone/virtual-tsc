...
export class VirtualFileSystem {
	...
	public writeFile(filename: string, content: string, overwrite: boolean = false): void {
		log("vfs", `writeFile(filename: "${filename}", content: length ${content ? content.length : 0}, overwrite: ${overwrite}`, "silly");
		...
		if (!exists) {
			log("vfs", "  creating new file with version 1", "silly");
			this.files[filename] = {
				version: 1,
				content,
			};
			...
			log("vfs", `  updating file => version ${this.files[filename].version}`, "silly");
		}
	}
	...
	public fileExists(filename: string, suppressLog: boolean = false): boolean {
		...
		if (!suppressLog) log("vfs", `fileExists("${filename}") => ${ret}`, "silly");
		return ret;
	}
	...
	public deleteFile(filename: string): void {
		log("vfs", `deleteFile("${filename}")`, "silly");
	...
	public readFile(filename: string): string {
		...
		log("vfs", `readFile("${filename}") => length ${ret ? ret.length : 0}`, "silly");
		return ret;
	}
	...
	public getFileVersion(filename: string): number {
		...
		log("vfs", `getFileVersion("${filename}") => ${ret}`, "silly");
		return ret;
	}
	...
	public getFilenames(): string[] {
		log("vfs", `getFilenames()`, "silly");
		return Object.keys(this.files);
	}

	public getDirectories(root: string): string[] {
		log("vfs", `fs.getDirectories(${root})`, "silly");
		let paths = this.getFilenames();
		log("vfs", `fs.getDirectories => paths = ${paths}`, "silly");
